...
libraries, a small readable codebase. The mature frameworks (MLX, Ollama/llama.cpp)
are faster — millfolio trails them by roughly 3× on decode and several× on prefill —
and closing that gap is the interesting part.
...
- **The kernel-level deep dive** — the int4 GEMV bandwidth analysis, the
  ...
  [ARCHITECTURE.md](ARCHITECTURE.md); raw benchmark runs are in
